...
> 작성일: 2026-07-11  
> 문서 상태: Phase 1 계산 계약 확정 및 Phase 2 프로젝트·조직 설정 계약 반영 — 규칙 버전 `1.0.0`
...
| 필드 | 의미 |
|---|---|
| `rulesetVersion` | Phase 1 확정 규칙 버전 `1.0.0` |
| `commissionTiers` | 오름차순 `[300, 700, 1500, 2400, 6000, 20000, 60000]` |
| `pvpTargetByLevel` | 레벨 1은 2,400, 레벨 2는 1,500, 레벨 3 이상은 700 |
...
| `lowerLevelCommissionPreference` | 레벨 4 이상에게 약 8회 권장. 하드 제약이나 추가 PV 사유가 아님 |
...
- 이름은 중복될 수 있지만 `memberKey`와 `memberId`는 프로젝트 안에서 유일해야 한다.
- 레벨은 조직 깊이와 별개인 1 이상의 정수 입력값이다.
...
모든 회원은 비어 있지 않은 이름·회원 ID·유효한 레벨과 OpeningState를 가져야 한다. 조직 또는 시작값을 바꾸는 작업은 기존 객체를 부분 수정하지 않고 새 스냅샷을 만든다. Phase 2의 `IN_PROGRESS` 설정 단계에서는 자유롭게 편집할 수 있지만, 성공한 변경은 현재 활성 설정 묶음을 즉시 무효화하고 다시 검증한다. 계획·확정·실제 리비전이 생긴 뒤 새 스냅샷과 기존 리비전을 연결하는 방식은 Phase 5의 **Q-PRODUCT-02**에서 결정한다.
...
적용량에는 보름 PVP 시작값과 이번 보름 신규 PVP 전액을 포함한다. 레벨 목표를 초과한 양도 자르지 않는다. 프로젝트 시작 `dailyCarryPvp`는 제외하며, 신규 PVP에서 파생된 일일 이월 상태는 별도 PV로 재합산하지 않는다. 보름 PVP 시작값은 해당 회원 본인의 판정에만 사용하고 상위 조직에 신규 실적으로 재전파하지 않는다.
...
4. 같은 PV 안에서 대상 레벨 4 이상 회원의 커미션 날짜 분산
5. 대상 레벨 4 이상 회원의 8회 권장 목표와의 차이 최소화
6. 100의 배수 배치 선호
7. 완전 동률일 때의 결정적 정렬
...
| **Q-CALC-05** | 레벨은 1 이상의 정수다. 목표는 레벨 1 2,400, 레벨 2 1,500, 레벨 3 이상 700이다. 레벨 4 이상에는 약 8회 분산을 권장하며 레벨은 조직 깊이와 무관하다. | 향후 레벨 5 이상도 규칙 변경 없이 처리 | 2026-07-11 |
...
| **Q-SIM-03** | 같은 PV로 9회 이상 커미션이 가능하면 계속 최대화하는가, 8회까지만 보상하고 초과는 중립인가? | 레벨 4 이상 분산 목적 |
...
| **Q-PRODUCT-02** | Phase 2 설정 묶음 이후 계획·확정·실제 리비전이 있는 상태에서 조직·레벨·시작값 변경을 언제 허용하는가? 새 조직 스냅샷과 기존 리비전의 관계는 무효화, 복제, 재계산 중 무엇인가? | Phase 5의 계획 이후 조직 변경과 재계산 |
