...
> 작성일: 2026-07-11  
> 최종 갱신일: 2026-07-31
> 문서 상태: Phase 4 V3 기본 몫·비루트 총합·조직 경계 탐색 반영, Phase 6A 클라우드 안전 저장·복구 보강 구현 반영 — 규칙·엔진 `8.0.0`, 자동 정책 `11.0.0`, 목적함수 `10.0.0`
> Phase 1 결정 확정일: 2026-07-11
> Phase 4 Q-SIM-01~08 확정일: 2026-07-12
...
- 회원별 회사 누적 PVP 달성값. 0~2,400이며 qualification과 개인 PVP 목표 부족분의 공통 시작값이다. 보름 좌·우 적용량에는 포함하지 않는다.
- 회원별 일일 좌·우 시작 잔액. 일일 PVP 시작 잔액은 제품 계약상 항상 0이다.
...
| 필드 | 의미 |
|---|---|
| `rulesetVersion` | 누적 PVP 상한, 회원별 보름 좌·우 목표, 보름 PVP 원천, 순수 재귀 목표와 수당 환산 계약을 포함한 현재 규칙 버전 `8.0.0` |
...
| `allowedFortnightSideTargets` | 회원별 선택 가능 좌·우 목표 `[2500, 1500]` |
| `defaultFortnightSideTarget` | 새 기간에서 각 회원에게 적용하는 기본값 `2500` |
...
| `baseCommissionEntitlement` | 비루트 회원의 좌·우 목표가 각 1,500이면 환산 5회, 각 2,500이면 환산 8회. PVP 목표·깊이·이름과 무관하며 `M+30`을 넘기는 사유가 아님 |

확정 상수를 코드 여러 곳에 복제하지 않고 버전이 있는 규칙 집합으로 계산 함수에 전달한다. 작업표 목표는 `MAX(0, 선택 PVP 목표 - 누적 PVP 시작값)`과 각 회원의 `fortnightSideTarget`을 조직 하위부터 순수 재귀 합산하는 파생값이며 별도 `rootFortnightSideTarget` 필드는 두지 않는다. 자동 계획도 회원별 1,500/2,500과 혼합 조직을 같은 값으로 계산한다. 비루트 회원의 기본 몫은 `fortnightSideTarget`만으로 1,500→5회, 2,500→8회로 정하고, 별도로 목표 보존 총량과 조직 구조에서 추가 기회의 이론상 환산 상한을 계산한다.
...
| 자동 계획 request | `4.0.0` |
| 자동 계획 policy | `11.0.0` |
| 자동 계획 objective | `10.0.0` |
| calendar contract | `1.0.0` |
| problem fingerprint | `4.0.0` |
| incumbent checkpoint | `5.0.0` |
| worker protocol | `5.0.0` |
...
| `timezone` | 계획의 고정 업무 시간대. 현재 제품 기본값은 `America/Sao_Paulo` |
...
| `fortnightSideTarget` | 사용자가 이번 기간에 선택한 좌·우 각각의 목표 `2500 | 1500` |
...
- PVP 목표, 좌·우 목표와 찾기용 표지판은 어떤 계산에서도 서로를 파생하지 않는다.
...
엔진은 감사 가능한 계산 역할을 위해 다음 다섯 필드를 유지한다.

| 필드 | 계산 용도 |
|---|---|
| `openingQualificationPvp` | 제품에서 입력한 회사 누적 PVP. 당일까지의 직접 신규 PVP와 합산해 300 gate를 판정 |
| `fortnightPvpOpeningCredit` | 같은 회사 누적 PVP. 개인 PVP 목표 부족분에만 사용하며 보름 마감 작은 쪽 적용과 상위 조직 전파에서는 제외 |
| `dailyCarryPvp` | 제품 계약상 항상 0인 첫 계산일의 일일 PVP 잔액 |
| `dailyCarryLeft` | 첫 계산일의 일일 좌 잔액 |
| `dailyCarryRight` | 첫 계산일의 일일 우 잔액 |

제품 설정 UI의 `PVP 시작값`은 이전 계획의 일일 잔액이 아니라 회사 시스템에서 영구 누적된 달성 현황이다. 0~2,400만 허용하고 Phase 2 정규화 어댑터가 같은 값을 `openingQualificationPvp`와 `fortnightPvpOpeningCredit`에 명시적으로 기록하며 `dailyCarryPvp`는 0으로 기록한다. `dailyCarryLeft/Right`만 사용자가 첫날 계산에 필요한 값을 직접 입력한다. 몇 달 뒤 새 프로젝트를 만들 때도 회사 누적 PVP를 다시 입력하며, 앱이 이전 프로젝트의 종료값을 자동 이월하지 않는다.

내부 필드는 계산 역할이 다르므로 이후 장부에서는 서로 대신 사용하지 않는다. 다만 현재 제품 입력 계약에서는 qualification과 개인 PVP 목표 시작값이 같은 회사 누적값이라는 불변조건을 가진다. 자동 계획은 이번 기간 신규 PVP를 이 누적값에 더했을 때 2,400을 넘는 값을 배정하지 않는다. 수동 계획과 실제값은 불가피한 초과 입력을 경고로 알리되 입력값 그대로 계산한다. 보름 좌·우 적용량은 누적 시작값을 제외한 이번 기간 신규 PVP만 사용한다.
...
Phase 1 수동 계획과 실제값은 회사에서 실제 발생한 값을 기록하기 위해 1 PV 단위 정수를 계속 허용한다. Phase 4가 새로 만드는 자동 후보의 직접 `PVP/SELF LEFT/SELF RIGHT` 셀만 `0` 또는 `30 이상`이어야 한다. 이는 계산 엔진의 일반 숫자 형식이 아니라 자동 계획의 운영 가능성 제약이다.
...
300 미만이어도 양쪽이 공식 단계에 도달하면 회사 정산 결과와 같은 실제 초기화를 수행한다. 이를 full commission으로 표시하거나 수당 발생일·환산 횟수로 세지 않는다. 자동 계획 후보에 이 사건이 있으면 verifier가 후보를 거부하며, 수동 계획은 계산 결과와 reset trace를 보존한 채 경고한다.
...
- 보름 PVP 적용 방향과 적용량
- 적용 전·후 좌·우
- 회원이 선택한 좌·우 각각의 2,500 또는 1,500 달성 여부
- 전체 보름 목표 달성 여부
- qualification-valid full commission 발생일 수, 단계별 내역과 300단계 기준 환산 횟수
- below-300 실제 정산과 blocking warning 내역
- PVP 목표 700이면서 좌·우 각 2,500인 회원의 환산 8회 권장 상태
...
- `openingQualificationPvp`: 제품의 회사 누적 PVP를 qualification 역할로 정규화한 값이다. 매일 그날까지의 직접 신규 PVP와 inclusive 합산하지만 일일 정산 reset과 조직 전파에는 사용하지 않는다.
- `fortnightPvpOpeningCredit`: 같은 회사 누적 PVP를 개인 목표 역할로 정규화한 값이다. 개인 PVP 목표 부족분에만 사용하고 본인의 보름 작은 쪽 적용이나 상위 회원의 이번 보름 좌·우에는 신규 실적으로 사용하지 않는다.
- 프로젝트 시작 입력인 `dailyCarryPvp/Left/Right`: 해당 회원의 첫 일일 장부에만 넣고 부모의 신규 원본이나 보름 원본에 다시 전파하지 않는다. 현재 제품에서 `dailyCarryPvp`는 항상 0이고 LEFT/RIGHT만 사용자가 입력한다.
...
첫 정산일의 `carryIn`은 회원의 `dailyCarryPvp/Left/Right`다. 현재 제품 요청의 PVP는 0이며 좌·우 시작 잔액만 첫날에 들어온다.
...
자동 계획에서 `fortnightPvpOpeningCredit + newPvpTotal`은 회사 누적 상한 2,400을 넘을 수 없다. 시작값이 2,400이면 자동 `newPvpTotal`은 반드시 0이고, 시작값이 1,600이면 자동 계획이 배정할 수 있는 신규 PVP headroom은 최대 800이다. 선택 목표를 이미 달성한 회원도 총 신규 PV를 늘리지 않는 동률 계획 안에서는 남은 headroom을 미래 달성 투자로 사용할 수 있다. 수동 계획과 실제값의 초과 입력은 경고와 함께 계산하며 자동 후보의 유효성에는 영향을 주지 않는다. 이 투자는 새 프로젝트에 앱이 자동 이월한다는 뜻이 아니라 회사 누적 현황이 증가한다는 뜻이다.

### 9.3 마감 PVP의 작은 쪽 적용

확정 공식은 다음과 같다.

- `periodPvpForSide = newPvpTotal`
- 원본 누적 좌·우 중 작은 쪽에 `periodPvpForSide` 전량을 한 번 적용
- 원본 누적 좌·우가 같으면 왼쪽을 선택
- 적용 후 좌·우가 해당 회원의 `fortnightSideTarget` 이상인지 판정
...
각 회원은 좌·우 각각 2,500 또는 1,500을 직접 선택하며 새 기간의 기본값은 2,500이다. 작업표 목표 행은 회원별 `MAX(0, selectedPvpTarget - fortnightPvpOpeningCredit)`과 해당 회원의 `fortnightSideTarget`을 하위부터 순수 재귀 합산한다. 맨 위 회원도 같은 재귀값을 그대로 표시하며 22,500 고정 하한이나 회원 수별 조회표를 적용하지 않는다.

PVP 적용은 최종 판정용 파생값이며 원본 누적 좌·우를 변경하거나 다음 프로젝트 시작값을 자동 생성하지 않는다.

### 9.4 Qualification trace, 수당 발생일과 환산 횟수

`RunningFortnightState`는 반월 PVP 평가와 별도로 날짜별 `qualificationPvp`를 보존한다. `FortnightAssessment.commissionOccurrences`와 `commissionDays`에는 `settlementKind === FULL_COMMISSION`인 날짜만 포함한다. Below-300 settlement, skipped day와 단계 미발생일은 0일이다. 정상 full tier는 높이에 관계없이 그 날짜에 발생일 1일로 센다.

`FortnightAssessment.commissionEquivalentUnits`는 300단계 수당 금액을 1회로 본 운영 환산값이다. 확정된 매핑은 `300→1`, `700→2`, `1,500→4`, `2,400→8`이다. 따라서 700단계 4일은 `commissionDays=4`, `commissionEquivalentUnits=8`이다. 6,000·20,000·60,000 단계는 수당 금액과 환산 계약이 없으므로 해당 단계가 포함된 전체 환산값은 `null`이며 UI는 미확정으로 표시한다. 자동 계획은 같은 이유로 순위 계산을 fail-closed한다.

PVP 목표 700 회원의 counted day는 다음 조건을 모두 만족한다.

```text
member.pvpTarget === 700
&& member.fortnightSideTarget === 2500
&& settlementKind === FULL_COMMISSION
&& qualificationPvp >= 300
```

좌·우 각 1,500을 선택한 회원은 2,500 전용 8회 권장 인원 통계에는 포함하지 않는다. 다만 일반 목표 700 공평성 집단에는 포함하며, 최소 목표 보존 총량과 하위 조직 구조로 계산한 이론상 환산 상한과 실제 환산값의 미달을 비교한다. 하위 조직이 없는 최소 총량 회원은 5회 상한이지만 이를 별도 하드 조건으로 만들지 않는다.
...
| `simulatePlan` | 정규 자동 계획 요청·policy `11.0.0`·solve control | verified candidate, 맨 위 회원 목표·미달 안내, proof progress와 `RUNNING/OPTIMAL/TIME_LIMIT/CANCELLED/INFEASIBLE/FAILED` 상태 |
| `resimulateFuture` | 실제 스냅샷·고정 경계·미래 기준안·규칙 | 미래 후보, 차이, 검증 결과 |

앞의 다섯 계약은 cumulative-PVP-cap-aware ruleset·engine `8.0.0`의 계산 권위다. Phase 4 자동 계획은 이를 독립 verifier로 호출하며, 부분 재시뮬레이션은 이후 Phase에서 같은 계산 코어를 호출한다.
...
- ruleset·engine `8.0.0`
- automatic-plan request·problem fingerprint·model·model certificate `4.0.0`
- automatic-plan checkpoint·worker protocol `5.0.0`
- automatic-plan policy `11.0.0`, objective `10.0.0`, calendar `1.0.0`
- 제품 실행 제한 상수 `AUTOMATIC_PLAN_PRODUCT_TIME_LIMIT_MS = 1_800_000`

정규 요청은 canonical date-only 전체 반월 날짜 목록과 skip 집합, 루트부터 LEFT 후 RIGHT로 순회한 canonical 회원 키 목록, 회원별 회사 누적 PVP와 내부 opening 매핑, 문제 fingerprint를 명시한다. Policy `11.0.0`은 고정된 `deterministicSeed`와 전체 영업일을 사용하는 deterministic constructive warm start를 가지며, 각 회원의 `fortnightSideTarget`을 후보 총량·기본 몫·환산 상한에 적용한다. 후보 생성은 300·200·400 등 날짜 profile, 하위 가지 동기화·회전, 총량 보존 경계 이동, 회원별 specialist를 보존하는 bounded beam, 좌·우 가지와 본인 PVP를 함께 다음 300/700/1,500/2,400 경계로 옮기는 조직 단위 이동, 최대 30 PV 마무리 보정을 순서대로 사용한다. 이동 좌표는 그 회원과 루트까지의 영향 경로를 추적하며, 보호 모드에서는 옮기기 전 날짜에 해당 경로가 이미 받던 단계를 떨어뜨리지 않는다. 공평성 교환을 찾는 제한된 완화 모드도 함께 평가한다. 20명 이하에서는 비루트 전 회원의 초기 날짜 profile을 만들고, 20명을 넘으면 좌·우 목표가 만드는 기본 몫, 구조상 환산 상한, canonical 순서로 12명만 초기 profile을 만든 뒤 현재 미달이 큰 최대 12명을 다시 고른다. 전 회원의 목적값은 계속 계산하며 이 상한은 후보 생성 예산일 뿐 회원을 평가에서 제외하는 규칙이 아니다. 모든 결과는 휴리스틱 후보이며 exact·optimal 주장을 하지 않는다. 이전 policy/objective 후보·checkpoint·worker 메시지는 호환되지 않는다. 제품 정책에는 `runMode`, 사용자 선택 `timeLimitMs`, 3시간·custom duration을 두지 않는다.

자동 계획 요청은 모든 활성 회원의 `fortnightSideTarget`이 1,500 또는 2,500인지 검증하고 두 값을 회원별로 그대로 받는다. Constructive 후보와 member/root capacity는 하위부터 각 회원의 선택 목표를 사용하며, canonical Phase 1 재계산이 최종 `allTargetsMet`를 다시 확인한다. 한 조직 안에 1,500과 2,500이 섞여 있어도 같은 경로를 사용한다.

### 12.2 결정 변수와 candidate shape

자동 배정 가능한 각 영업일·회원에 대해 다음만 안전 정수 변수로 둔다. 자동 후보의 각 직접값 domain은 `0` 또는 `30 이상`이며, 수동·실제 계산 엔진은 정확한 1 PV 값을 계속 허용한다.

- 신규 PVP
- 왼쪽이 `SELF`일 때의 신규 좌 PV
- 오른쪽이 `SELF`일 때의 신규 우 PV

연결 방향은 변수가 아니다. 해당 값은 하위 서브트리 변수의 합으로 계산한다. Candidate는 모든 canonical 날짜·회원 쌍에 정확히 한 셀을 가지며 PVP와 모든 editable SELF 필드를 0까지 포함한다. CHILD 필드는 구조적으로 없어야 한다. 중복, 누락, unknown identity·field, 비정규 순서, 문자열, 소수, `NaN`, 무한대, unsafe integer, overflow, 음수, negative zero와 1~29 직접값은 Phase 1 호출 전에 거부한다. 30 이상 값 중 39 같은 비정형 값은 하드 제약과 더 높은 목적에 필요하면 허용한다.

### 12.3 하드 제약

Canonical 달력에서 `SKIP_NO_INPUT`이 아닌 실제 입력 가능 영업일 수를 `N`으로 둔다. 맨 위 회원의 총량상 정상 커미션 목표 `U`는 다음 파생값으로 요청마다 계산한다.

```text
L = recursiveMinimumRawLeftTarget(root)   // opening 제외
R = recursiveMinimumRawRightTarget(root)  // opening 제외
P = requiredNewPvp(root)  // cumulative opening, selected target,
                          // lifetime 2,400 cap과 qualification 매핑 반영
OL = root.dailyCarryLeft
OR = root.dailyCarryRight

C0(L, R, P, M) = MAX n, where n is an integer and 0 <= n <= M:
    floor(L / 300) + floor(R / 300) >= n
    && MAX(0, 300n - L) + MAX(0, 300n - R) <= P
```

`OL/OR`은 `L/R`에 합산하지 않는다. 시작 잔액은 첫 full commission 한 번에만 사용할 수 있고 그날 초과분까지 reset되어 여러 날짜로 분할할 수 없기 때문이다. Phase 1의 일일 PVP 작은 쪽 적용, qualification, tier와 reset 의미로 첫 정상 커미션을 만드는 기간 신규 소비 tuple 집합 `F={(p0,l0,r0)}`를 exact bounded enumeration으로 파생한다. 각 tuple은 `0<=p0<=P`, `0<=l0<=L`, `0<=r0<=R`이고 opening을 포함한 첫날 계산이 qualification-valid full commission이어야 한다.
...
따라서 `OL=OR=10,000`이어도 opening만으로 늘릴 수 있는 일수는 최대 한 번이다. `U=N`이면 맨 위 회원이 모든 입력 가능 영업일에 정상 커미션을 받는 것이 운영 목표다. 총량 자체가 부족한 극소 조직은 `U<N`인 이론상 최대를 목표로 한다. 이 계산은 첫 reset과 남은 최소 목표 총량을 반영한 상한이다. 순수 재귀 최소 목표에서 구조상 최소 총 신규 PV `M`을 별도로 계산하고 자동 후보의 절대 상한은 `M+30`으로 제한하므로, `U`나 수당을 위해 100 PV를 추가 구매할 수 없다. 또한 이후 날짜별 carry, 자동 직접값 0 또는 30 이상과 조직 배치까지 만족하는 전체 일정의 존재를 증명하지 않으므로 exact solver의 feasibility proof나 후보 거부 조건으로 사용하지 않는다.

표시 계약은 `businessDayCount=N`, `targetCommissionDays=U`, `actualCommissionDays=qualification-valid root full-commission 일수`, `shortfallDays=max(0,U-actualCommissionDays)`, `capacityLimited=(U<N)`, `met=(shortfallDays===0)`이다.
...
- 모든 회원의 보름 assessed LEFT와 RIGHT가 각 회원이 선택한 1,500 또는 2,500 이상이어야 한다.
- 모든 canonical Sunday와 `SKIP_NO_INPUT` 날짜의 신규 배정은 0이다.
- 자동 직접 입력은 0 또는 30 이상이다.
- 모든 직접 신규 PV 합계는 구조상 최소 총량 `M+30` 이하이다. 이미 30 이상인 셀에는 1~30 PV를 더해 끝자리 보정을 할 수 있고 0인 셀을 새로 만들 때는 최소 30 PV여야 한다.
- 연결된 CHILD 방향에는 직접 배정하지 않는다.
- 조직 전파, 일일 smaller-side PVP 적용, tie-left, 최고 tier 하나, carry, 실제 reset, 보름 장부와 수당 환산을 ruleset `8.0.0` 그대로 적용한다.
...
`U` 계산에 사용하는 총량상 capacity 외에는 safe-integer 범위 밖의 일일·회원·셀별 운영 cap이 없다. 모델 성능을 위해 임의 cap을 만들지 않는다. Phase 4에는 수동 셀 잠금, 실제값, 확정 계획, fixed past boundary와 cross-period fairness가 없다.
...
1. **비루트 기본 몫 미달 최소화.** 좌·우 목표 1,500은 5회, 2,500은 8회로 정하고 `max(0, 기본 몫 - 실제 환산 횟수)`를 큰 순서로 정렬한 `nonRootBaseEntitlementDescendingEquivalentUnitShortfallVector`를 사전식 최소화한다. PVP 목표·깊이·이름·화면 마커는 쓰지 않는다.
2. **비루트 전체 환산 횟수 최대화.** 모든 비루트 회원의 실제 1·2·4·8 환산값 합인 `nonRootCommissionEquivalentUnits`를 최대화한다. 비루트 합계를 하나 줄여 맨 위 회원이나 특정 회원의 추가 몫을 늘리는 계획은 이 단계에서 진다.
3. **맨 위 회원 목표 미달 일수 최소화.** `rootCommissionGoalShortfallDays = max(0, U - rootFullCommissionDays)`를 최소화한다. shortfall fallback을 유효하지 않다고 버리지는 않는다.
4. **확인된 실제 수당 총액 최대화.** `M..M+30` 하드 범위 안에서 qualification-valid full commission에 `300→60,000원`, `700→120,000원`, `1,500→240,000원`, `2,400→480,000원`을 적용해 합산한다. 공식 6,000·20,000·60,000 단계가 하나라도 나오면 `AUTOMATIC_PLAN_PAYOUT_TABLE_INCOMPLETE`로 fail-closed한다.
5. **비루트 구조상 추가 기회 미달 최소화.** 비루트 전 회원의 `max(0, 구조상 이론상 환산 상한 - 실제 환산 횟수)`를 큰 순서로 정렬한 `nonRootStructuralOpportunityDescendingEquivalentUnitShortfallVector`를 사전식 최소화한다. 같은 합계에서 베로니카 9·Kelly 19처럼 추가 기회를 고르게 나눈 계획을 베로니카 10·Kelly 18처럼 몰린 계획보다 우선할 수 있다.
6. **총 직접 신규 PV 최소화.** 1~5번이 같을 때 PVP와 editable SELF LEFT/RIGHT 직접 셀을 각각 한 번만 센다. 전파된 조직 합계를 다시 세지 않는다.
7. **qualification-valid full commission의 일일 discarded excess 총합 최소화.** `prePvp + preLeft + preRight - 2 × commissionTier`를 합산한다.
8. **추가 비용 없는 미래 누적 PVP 투자 최대화.** 1~7번이 모두 같은 상태에서 회원별 `max(0, closingCumulativePvp - max(openingCumulativePvp, selectedPvpTarget))`의 합을 최대화한다.
...
`M+30`은 목적함수로 교환할 수 없는 하드 상한이다. 이 범위를 넘는 +31 또는 +100 PV 후보는 기본 몫이나 수당이 늘어도 verifier가 거부한다. 범위 안에서는 비루트 기본 몫·비루트 합계·맨 위 목표·수당·구조상 공평성을 순서대로 보존한 뒤 총 PV를 줄인다.

공동 하위 기여는 독립 목적이 아니다. 더 깊은 직접 입력이 소유 회원과 여러 상위 목표를 함께 채우면 실제 수당·총 PV와 뒤 목적에 자연스럽게 반영된다. Exact PVP 100 자체를 보상하거나 PVP-100 셀 수를 최대화하지 않는다. 자동 계획은 최종 직접값 1~29를 만들지 않지만 이미 30 이상인 셀의 1~30 PV 끝자리 증가는 허용하며 수동·실제 엔진은 정확한 1 PV 값을 계속 기록할 수 있다. 30 이상인 39 같은 정확 보정은 같은 수당의 100 배수 계획보다 총 PV를 줄일 때 우선한다.
...
3. 직접값이 0 또는 30 이상인지, 회원별 신규 PVP가 `2400 - openingCumulativePvp` headroom 안인지 확인한다.
4. 모든 final `allTargetsMet`과 날짜별 qualification trace를 확인한다.
5. `BELOW_QUALIFICATION_SETTLEMENT`가 하나라도 있으면 자동 후보를 거부한다.
6. 순수 재귀 최소 목표에서 `M`을 재계산하고 총 직접 신규 PV가 `M+30`을 넘으면 `AUTOMATIC_PLAN_EXCESS_PURCHASE_LIMIT_EXCEEDED`로 거부한다.
7. 요청의 `N/L/R/P/U`를 재계산하고 맨 위 회원의 full-commission 일수와 `rootCommissionGoalShortfallDays`를 계산한다. Shortfall은 후보 거부 사유가 아니라 첫 비교값과 사용자 안내다.
8. known payout table로 모든 발생 단계의 수당을 평가할 수 있는지 확인한다. 미확인 상위 단계가 있으면 후보 유효성과 별개로 자동 순위 계산을 fail-closed한다.
9. canonical objective와 display metric을 CalculationResult에서 checked integer로 다시 계산한다.
10. Solver score와 하나라도 다르면 model-consistency failure로 처리하고 raw vector를 노출하지 않는다.
...
Constructive candidate가 `U`일을 채우지 못했더라도 나머지 하드 계산 계약을 통과하면 verified shortfall candidate다. UI는 `맨 위 회원 전체 영업일 목표 미달 X일`을 표시하고 탐색을 계속한다. 총량상 목표를 만족하는 일정을 아직 찾지 못했다는 사실은 `INFEASIBLE` 증명이 아니다.
...
Phase 6A에서는 기존 Supabase 프로젝트의 계정·동기화형 저장을 사용한다. 계획의 업무 시간대는 `America/Sao_Paulo`로 고정하며 Supabase 리전이나 접속 기기의 시간대에서 자동으로 파생하지 않는다. 원격 최신 작업본은 전체 계획 단위로 덮어쓰고, IndexedDB는 오프라인 복구용 로컬 사본으로만 사용한다. 실사용 중 기간을 잘못 바꿔 날짜별 수동 입력이 사라진 사건에 따라 2026-07-31 보존 계약을 보강했다. 셀별 수정 이력을 무한히 만들지는 않되, 15분·7일 순환 보관본, 중요 작업 직전 보관본, 상파울루 날짜별 하루 보관본을 함께 유지한다.

### 14.2.1 Phase 6A 구현 경계

| 경계 | 구현 |
|---|---|
| 원격 원본 | `public.ngplan_projects`의 `document jsonb`에 `CloudPlanDocumentV2` 전체를 저장한다. 서버 trigger가 revision, UTC 저장 시각과 사용자 식별자를 정한다. |
...
| 원격 어댑터 | `src/cloud/supabase-plan-repository.ts`가 목록·읽기·upsert·숨김/복원과 보관본 읽기·제한된 안전 보관 RPC를 제공하며 DELETE 메서드를 노출하지 않는다. 안전 보관 RPC는 행을 잠그고 클라이언트가 방금 저장한 예상 리비전과 현재 서버 리비전이 같을 때만 보관한다. |
| 제품 진입점 | `src/ui/CloudApp.tsx`가 기존 Google Auth 세션, 계획 목록과 편집기, 시작 기간 확인, 보관본 사본 열기와 저장 상태를 조합한다. 프로덕션 진입점은 과거 `localStorage` 자료를 가져오지 않는다. |
...
서로 다른 기기 사이의 충돌은 합의한 마지막 도착 전체 문서 우선 정책이다. 한 기기에서는 원격 요청을 직렬화해 오래된 요청이 더 최신 입력 뒤에 완료되어 최종본을 되돌리는 일을 막는다. 계획 목록은 최근 순환·중요 작업·하루 보관본을 구분해 보여 주며, 사용자가 선택한 보관본은 원본을 바꾸지 않는 새 계획 사본으로 연다.
...
Phase 4의 `problemFingerprint 4.0.0`은 자동 계획 checkpoint, candidate identity와 warm-start compatibility의 필수 계약이다. 정규 bundle과 회사 누적 PVP·daily-zero 매핑, 회원별 `fortnightSideTarget`, canonical member sequence, canonical date/skip 집합, ruleset `8.0.0`, policy `11.0.0`, objective `10.0.0`, calendar `1.0.0`과 관련 schema version을 키 정렬된 canonical serialization으로 만든 뒤 고정된 hash 알고리즘을 적용한다.
...
- 같은 상한에서 실제 환산 `[7,7,7]` 대 `[0,8,8]`, 서로 다른 구조 상한, 빈 비루트 vector, 환산 8 대 9와 확정 tier 1·2·4·8을 검증한다.
- 구조상 최소 총량 `M+31` 이상인 후보는 기본 몫이나 수당이 늘어도 거부되는지, `M+30` 안에서는 비루트 기본 몫·비루트 합계·root shortfall·known payout·통합 구조상 미달 vector·총 PV·cost-neutral 미래 PVP 투자가 순서대로 적용되는지 검증한다.
- 자동 직접값 0/30 경계, 39 exact 보정, non-100 셀 수, max direct PVP와 deterministic allocation vector의 엄격한 우선순위를 검증한다.
- 공동 기여, exact PVP 100과 PVP-target-group 통계가 독립 목적 단계가 아님을 검증한다.
- same-date qualification crossing, one-sided pre-qualification carry와 below-300 후보 거부를 검증한다.
- 누적 PVP 2,400 cap/headroom, 시작 2,400 회원의 PVP 0, `N=13`, `N=14`, 총량 부족 `U=8`, root shortfall fallback과 미확인 상위 수당표 fail-closed를 검증한다.
...
Phase 2 최초 범위에는 라우터, 전역 상태 라이브러리, 드래그앤드롭 라이브러리와 브라우저 저장소를 추가하지 않았다. Phase 3 실사용 테스트 이후에는 설정 화면과 수동 계획표를 오가거나 브라우저를 다시 열어도 입력이 남도록 현재 기기의 작업 초안 하나를 `localStorage`에 자동 저장한다. 현재 workspace schema는 v4이며 v1·v2·v3 초안의 회원별 좌·우 목표를 2,500으로 보완하고 파생 번들·자동 체크포인트는 폐기한 뒤 다시 검증한다. 원격 문서는 `CloudPlanDocumentV2`이고 v1을 같은 방식으로 마이그레이션한다. 사이트 데이터가 삭제되면 로컬 사본은 사라진다.
...
| ID | 확정 결정 | 영향 | 확정일 |
|---|---|---|---|
| **Q-CALC-01** | 개인 PVP는 `누적 PVP 시작값 + 이번 보름 신규 PVP`, 보름 작은 쪽 적용량은 이번 보름 신규 PVP 전액만 사용한다. `dailyCarryPvp`와 누적 PVP 시작값은 보름 좌·우 적용량에서 제외한다. | 개인 PVP와 보름 좌·우 판정 원천 고정 | 2026-07-17 |
...
| **Q-CALC-05** | 회원별 PVP 목표는 2,400·1,500·700 중 하나를 직접 선택한다. PVP 목표 700이면서 좌·우 각 2,500인 회원에게는 같은 PV 안에서 300단계 기준 환산 8회를 권장한다. 이름 앞 숫자와 색상은 계산과 무관한 선택적 찾기용 표지판이다. | 레벨 개념 제거, 목표·표지판·조직 위치·수당 환산 분리 | 2026-07-19 |
| **Q-CALC-06** | 프로젝트 하나는 루트 한 명의 연결된 비순환 트리다. 루트 없음·복수 루트·고아 회원은 오류다. | 조직 검증 계약 고정 | 2026-07-11 |
| **Q-CALC-07** | 각 회원은 이번 기간의 좌·우 목표를 각각 2,500 또는 1,500으로 선택한다. 새 기간은 모두 2,500으로 시작하고 보름 신규 PVP 전액을 작은 쪽에 적용해 선택 목표를 판정한다. 자동 계획은 회원별 1,500/2,500과 혼합 조직을 지원하며, 1,500 회원은 별도 고정 5회 대신 구조상 이론값으로 평가한다. | 회원별 목표 원본, 새 기간 초기화, 자동 후보·상한·검증의 혼합 목표 지원 고정 | 2026-08-02 |
...
| **Q-SIM-06** | 당시 총 PV 최소화를 첫 목적으로 확정했다. exact PVP 100을 독립 목적으로 두지 않고 일반 100배수·max PVP·tie-break만 뒤에 두는 결정은 유지한다. 1~3순위 비용 계약은 2026-08-02 Q-SIM-08이 대체한다. | 과거 objective `7.0.0` 결정과 현재 유지되는 후순위 가독성 계약 구분 | 2026-07-19 |
...
| **Q-SIM-08** | **Q-SIM-09로 대체된 역사 기록.** 엄마의 승인 수동안 `202608A 민경욱`을 정답지 회귀로 사용하고 `M+30` 상한, 조직 경계 탐색, 결정성을 고정했다. 당시 PVP 목표 집단별 objective `9.0.0` 순서는 더 이상 활성 계약이 아니다. | Policy 10/objective 9의 역사적 근거 보존 | 2026-08-02 |
| **Q-SIM-09** | Q-SIM-03/08의 PVP 목표 집단 우선순위를 대체한다. 비루트 회원의 좌·우 목표 1,500은 기본 5회, 2,500은 기본 8회다. 기본 몫 미달 벡터, 비루트 전체 환산 합계, root shortfall, 확인 수당, 비루트 전 회원의 구조상 추가 기회 미달 벡터, 총 신규 PV 순으로 비교한다. PVP 목표·조직 깊이·이름·UI 마커는 우선 집단을 만들지 않는다. Policy 11.0은 루트 경계 후보와 이동 좌표에서 조상까지의 영향 경로 보호를 추가하며, objective 10.0·checkpoint 5.0·worker protocol 5.0으로 이전 저장 후보를 무효화한다. `202608A` 정답지는 production 셀 하드코딩이 아니라 비교 평가 자료다. 현재 결정적 휴리스틱은 모든 기본 몫을 지키고 비루트 77회를 찾지만 정답지의 78회보다 한 회 부족하므로, 평가기와 남은 후보 생성 병목을 구분해 기록한다. | 운영자가 말한 “자기 몫”과 아래 회원 우선 원칙을 V3 계약으로 고정 | 2026-08-03 |
| **Q-SIM-10** | 베로니카 좌 시작값 0인 역사 회귀를 보존하고 정정값 235를 별도 fixture로 둔다. 정정 조건에서 승인 수동안은 비루트 79회·6,060,000원, Policy 11 V3는 모든 기본 몫을 지킨 비루트 78회·6,000,000원이다. 235는 두 계획을 각각 한 회 올릴 뿐 Kelly의 19 대 18 차이를 만들지 않는다. Kelly의 900 실적을 700과 다음 300 경계로 옮기는 제한 탐색은 Kelly 19회 후보를 만들지만 박진숙·김길주 등의 기본 몫을 훼손하고 비루트 합계를 개선하지 못한다. 따라서 이 국소 이동은 production에 추가하지 않고 Policy 11을 동결한다. | 실제 입력 정정과 알고리즘 개선을 분리하고, 안전하지 않은 국소 최적화를 배제 | 2026-08-04 |
...
| **Q-DATA-01** | 기존 Supabase 프로젝트와 기존 Auth를 재사용한다. 공개 가입은 끄고 두 허용 계정만 동일 workspace 자료를 읽고 쓴다. RLS를 모든 노출 테이블에 적용하며 DELETE는 클라이언트에 제공하지 않는다. | Phase 6A 인증·권한·원격 저장 |
| **Q-DATA-02** | 계획 업무 시간대는 `America/Sao_Paulo`로 고정한다. UTC timestamp와 상파울루 기준 `business_date`를 분리하며 기기 시간대는 계획 날짜를 바꾸지 않는다. | 날짜·하루 보관본·여러 기기 일관성 |
| **Q-DATA-03** | 최신 작업본은 전체 계획 단위로 마지막 도착 저장이 승리한다. IndexedDB는 오프라인 사본이다. 2026-07-31 실사용 사고 후 셀별 무한 이력 대신 15분×7일 순환 보관, 중요 작업 직전 최근 50개, 상파울루 날짜별 하루 보관을 결합했다. 복구는 원본 덮어쓰기 없이 새 계획 사본으로만 수행한다. | 자동 저장·오프라인 복구·제한된 복구 보존 범위 |
...
Phase 1 계산 규칙, Phase 2 설정·UI 스택, Phase 4 Q-SIM-01~08과 Phase 6A Q-DATA-01~03은 확정되었다. Phase 1 구현은 GitHub Issue #1에 연결해 완료했으며, Phase 4는 202608A 승인 수동안 회귀를 반영해 재개했다. Phase 5의 실제값·부분 재시뮬레이션과 전체 Phase 6의 종료 기록·외부 백업 범위는 별도 계약으로 남긴다.
