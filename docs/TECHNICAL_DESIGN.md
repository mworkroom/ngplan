...
> 작성일: 2026-07-11  
> 문서 상태: Phase 1 계산 계약 확정 — 규칙 버전 `1.0.0`  
...
모든 회원은 비어 있지 않은 이름·회원 ID·유효한 레벨과 OpeningState를 가져야 한다. 조직 또는 시작값을 바꾸는 작업은 기존 객체를 부분 수정하지 않고 새 스냅샷을 만든다. 어느 프로젝트 상태에서 이를 허용할지는 **Q-PRODUCT-02**에서 결정한다.
...
- 부모·위치가 다른 새 스냅샷을 계산하면 모든 파생 결과를 처음부터 다시 만든다. 기존 확정·실제 리비전을 언제 무효화할지는 **Q-PRODUCT-02**에서 정한다.
...
Phase 1은 다음 최소 구성으로 확정한다.

- TypeScript: 정수·상태·방향과 원본·파생 계약을 명시적으로 표현
- Vite: 정적 웹 앱의 개발·빌드 기반
- Vitest: 계산 사례 중심의 빠른 단위·통합 테스트
- UI 프레임워크: Phase 2 화면 복잡도와 J님의 선호를 확인한 뒤 결정
- 최적화 실행: Phase 4에서 UI 응답성을 위해 별도 작업 스레드 사용 검토

현재 저장소의 `index.html`은 초기 Hello World뿐이므로 기존 앱 프레임워크와의 호환 제약은 없다. UI 프레임워크는 Phase 2 착수 전에 별도로 결정한다.
...
| **Q-PRODUCT-02** | 초안·확정·실제가 있는 상태에서 조직·레벨·시작값 변경을 언제 허용하는가? 변경 시 새 스냅샷과 기존 리비전의 관계는 무효화, 복제, 재계산 중 무엇인가? | 프로젝트 상태 전이와 조직 편집 |

Phase 1 계산 규칙과 기술 스택의 차단 질문은 모두 해소되었고, 구현은 GitHub Issue #1에 연결해 완료했다. 17.2는 해당 기능을 포함하는 Phase 진입 전에 확정한다.
